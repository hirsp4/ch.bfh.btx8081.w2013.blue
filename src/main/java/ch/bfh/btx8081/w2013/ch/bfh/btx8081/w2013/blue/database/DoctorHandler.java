...
public class DoctorHandler {
	...
	public final static String OfficeName = "office:";
	 ...
	 */
	public DoctorHandler(String selectDatabase) {
		if (selectDatabase.equals("Doctor")) {
			this.DATABASE = new File("Doctor.txt");
		} else {
			this.DATABASE = null;
		}
	...
	private TreeMap<Integer, HashMap<String, String>> buildMapFromFile(File file)
		...
		while (s.hasNextLine()) {
			...
			hashMap.put(DID, splitted[9]);
			hashMap.put(OfficeName, splitted[10]);
	...
	private void writeMapToFile(String path,
		...
		while (itr.hasNext()) {
			...
			printer.print(";");
			printer.print(treeMap.get(i).get(DID));
			printer.print(";");
			printer.print(treeMap.get(i).get(OfficeName));
			printer.println();
		}
		printer.close();
	}
	 ...
	 */
	private TreeMap<Integer, HashMap<String, String>> searchDoctors(
		...
		while (itr.hasNext()) {
			...
			if (delemitter.equals(NAME)) {
				...
				if (data.equals(tm.get(i).get(delemitter)))
					resultMap.put(i, tm.get(i));
			} else if (delemitter.equals(DID)) {
				if (data.equals(tm.get(i).get(delemitter)))
					resultMap.put(i, tm.get(i));
			} else if (delemitter.equals(OfficeName)) {
				if (data.equals(tm.get(i).get(delemitter)))
					resultMap.put(i, tm.get(i));
			} else {
	 ...
	 */
	public ArrayList<Doctor> searchDoctors(String delemitter, String data)
			throws IllegalArgumentException, PersonNotFoundException {
		ArrayList<Doctor> foundPerson;
		TreeMap<Integer, HashMap<String, String>> treeFoundPerson = null;
		try {
			treeFoundPerson = this.searchDoctors(
	 ...
	 * @return ArrayList
	 */
	private ArrayList<Doctor> writeMapToList(
		...
		while (itr.hasNext()) {
			...
			int did = Integer.parseInt(tm.get(i).get(DID));
			...
			String officeName = tm.get(i).get(OfficeName);
			Doctor doctor = null;
			

			doctor = new Doctor(did, name, foreName, address, gender,
					birthdate, officeName);
	...
	public void addDoctor(Doctor doctor) throws IOException {
		...
		hashMap.put(FORENAME, doctor.getForeName());
		...
		hashMap.put(GENDER, String.valueOf(doctor.getGender()));
		hashMap.put(BIRTHDATE, doctor.getBirthdate());
		hashMap.put(DID, Integer.toString(doctor.getDid()));
		hashMap.put(OfficeName, doctor.getOfficeName());

		tm.put(doctor.getDid(), hashMap);
	 ...
	 * @throws PersonNotFoundException
	 */
	public Doctor getDoctor(String delemitter, String data)
			throws PersonNotFoundException {
		ArrayList<Doctor> tm = searchDoctors(delemitter, data);
		return tm.get(0);
	}
	 ...
	 * @throws PersonNotFoundException
	 */
	public void removePatient(Doctor doctor) throws FileIsEmptyException,
			IOException, PersonNotFoundException {
		TreeMap<Integer, HashMap<String, String>> tm;
		tm = buildMapFromFile(DATABASE);
		ArrayList<Doctor> list = searchDoctors(DID,
				Integer.toString(doctor.getDid()));
		if (list.isEmpty()) {
			throw new PersonNotFoundException();
		}
		tm.remove(list.get(0).getDid()); // In der Liste ist sicher nur ein
